...
router.post('/operation', auth, async (req, res) => {
	// Champs attendus : itemId, action, quantity, operatorId, reason
	try {
		const { itemId, action, quantity, operatorId, reason } = req.body;
		if (!itemId || !action || !quantity || !operatorId) {
		...
		await logStockMovement({ itemId, action, quantity, userId: req.user.id, operatorId, reason });
